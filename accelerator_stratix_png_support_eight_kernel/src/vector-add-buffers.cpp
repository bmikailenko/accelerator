...
template <int N> class ProducerKernel3;  // Forward declare kernel name
template <int N> class ProducerKernel4;  // Forward declare kernel name
...
template <int N> class ConsumerKernel3;  // Forward declare kernel name
template <int N> class ConsumerKernel4;  // Forward declare kernel name
...
using ProducerToConsumerPipe2 = sycl::ext::intel::pipe<
    ...
    1000>;
// PIPE DEFINITIONS
using ProducerToConsumerPipe3 = sycl::ext::intel::pipe<
    class ProducerConsumerPipe3,
    uint64_t,
    1000>;
using ProducerToConsumerPipe4 = sycl::ext::intel::pipe<
    class ProducerConsumerPipe4,
    uint64_t,
    1000>;
...
sycl::event Producer3(sycl::queue &q, sycl::buffer<uint64_t, 1> &a_buf, size_t width, size_t height) {

    auto e = q.submit([&](sycl::handler &h) {

        sycl::accessor a(a_buf, h, sycl::read_only);

        h.single_task<class ProducerKernel3<3>>(
            [=]() [[intel::kernel_args_restrict]] {
            ...
            for (size_t i = 0; i < height; i++) { // for each row
                for (size_t j = 0; j < iters_per_row; j++) {
                    ...
                    for (size_t x = 0; x < ELEMENTS_PER_DDR_ACCESS; x++) {
                        size_t idx = j * ELEMENTS_PER_DDR_ACCESS + x;
                        ProducerToConsumerPipe3::write(a[(i * width) + (width - 1) - idx]);
                    }
                }
            }
        });
...
sycl::event Producer4(sycl::queue &q, sycl::buffer<uint64_t, 1> &a_buf, size_t width, size_t height) {

    auto e = q.submit([&](sycl::handler &h) {

        sycl::accessor a(a_buf, h, sycl::read_only);

        h.single_task<class ProducerKernel4<3>>(
            [=]() [[intel::kernel_args_restrict]] {
            ...
            for (size_t i = 0; i < height; i++) { // for each row
                for (size_t j = 0; j < iters_per_row; j++) {
                    ...
                    for (size_t x = 0; x < ELEMENTS_PER_DDR_ACCESS; x++) {
                        size_t idx = j * ELEMENTS_PER_DDR_ACCESS + x;
                        ProducerToConsumerPipe4::write(a[(i * width) + (width - 1) - idx]);
                    }
                }
            }
        });
...
sycl::event Consumer3(sycl::queue &q, sycl::buffer<uint64_t, 1> &b_buf, size_t width, size_t height) {

    auto e = q.submit([&](sycl::handler &h) {

        sycl::accessor b(b_buf, h, sycl::write_only, sycl::no_init);

        h.single_task<class ConsumerKernel3<3>>(
            [=]() [[intel::kernel_args_restrict]] {
            ...
            [[intel::loop_coalesce(3)]]
                for (size_t i = 0; i < height; i++) { // for each row
                    for (size_t j = 0; j < iters_per_row; j++) {
                        ...
                        for (size_t x = 0; x < ELEMENTS_PER_DDR_ACCESS; x++) {
                            size_t idx = j * ELEMENTS_PER_DDR_ACCESS + x;
                            b[(i * width) + idx] = ProducerToConsumerPipe3::read();
                        }
                    }
                }
        });
...
sycl::event Consumer4(sycl::queue &q, sycl::buffer<uint64_t, 1> &b_buf, size_t width, size_t height) {

    auto e = q.submit([&](sycl::handler &h) {

        sycl::accessor b(b_buf, h, sycl::write_only, sycl::no_init);

        h.single_task<class ConsumerKernel4<3>>(
            [=]() [[intel::kernel_args_restrict]] {
            ...
            [[intel::loop_coalesce(3)]]
                for (size_t i = 0; i < height; i++) { // for each row
                    for (size_t j = 0; j < iters_per_row; j++) {
                        ...
                        for (size_t x = 0; x < ELEMENTS_PER_DDR_ACCESS; x++) {
                            size_t idx = j * ELEMENTS_PER_DDR_ACCESS + x;
                            b[(i * width) + idx] = ProducerToConsumerPipe4::read();
                        }
                    }
                }
        });
...
int main(int argc, char * argv[]) {
    std::vector<uint64_t> outdata_flat1, outdata_flat2, outdata_flat3, outdata_flat4;
    sycl::event producer_event1, producer_event2, producer_event3, producer_event4;
    sycl::event consumer_event1, consumer_event2, consumer_event3, consumer_event4;
    std::vector<uint64_t> indata_flat1, indata_flat2, indata_flat3, indata_flat4;
    ...
    for (size_t i = height/4; i < height/2; i++) {
        for (size_t j = 0; j < width; j++) {
            indata_flat2.push_back(static_cast<uint64_t>(indata[i][j]));
        }
    }
    for (size_t i = height/2; i < ((height/4)*3); i++) {
        for (size_t j = 0; j < width; j++) {
            indata_flat3.push_back(static_cast<uint64_t>(indata[i][j]));
        }
    }
    for (size_t i = ((height/4)*3); i < height; i++) {
        for (size_t j = 0; j < width; j++) {
            indata_flat4.push_back(static_cast<uint64_t>(indata[i][j]));
        }
    }
    outdata_flat1.resize(indata_flat1.size());
    outdata_flat2.resize(indata_flat2.size());
    outdata_flat3.resize(indata_flat3.size());
    outdata_flat4.resize(indata_flat4.size());
    ...
    try {
        ...
        if(command.compare("flip") == 0) {
            ...
            sycl::buffer producer_buffer2(indata_flat2, {sycl::property::buffer::mem_channel{1}});
            sycl::buffer producer_buffer3(indata_flat3, {sycl::property::buffer::mem_channel{2}});
            sycl::buffer producer_buffer4(indata_flat4, {sycl::property::buffer::mem_channel{2}});
            sycl::buffer consumer_buffer1(outdata_flat1, {sycl::property::buffer::mem_channel{3}});
            sycl::buffer consumer_buffer2(outdata_flat2, {sycl::property::buffer::mem_channel{3}});
            sycl::buffer consumer_buffer3(outdata_flat3, {sycl::property::buffer::mem_channel{4}});
            sycl::buffer consumer_buffer4(outdata_flat4, {sycl::property::buffer::mem_channel{4}});
            ...
            for (size_t repetition = 0; repetition < num_repetitions; repetition++) {
                ...
                producer_event3 = Producer3(q, producer_buffer3, width, indata_flat3.size()/width);
                producer_event4 = Producer4(q, producer_buffer4, width, indata_flat4.size()/width);
                ...
                consumer_event3 = Consumer3(q, consumer_buffer3, width, indata_flat3.size()/width);
                consumer_event4 = Consumer4(q, consumer_buffer4, width, indata_flat4.size()/width);
            }
        }
    ...
    // Unflatten first quarter of output data
    for (size_t i = 0; i < height/4; i++) {
        for (size_t j = 0; j < width; j++) {
    ...
    // Unflatten second quarter of output data
    for (size_t i = height/4; i < height/2; i++) {
        for (size_t j = 0; j < width; j++) {
            uint64_t val = outdata_flat2[((i-(height/4))*width)+j];
    ...
    // Unflatten third quarter of output data
    for (size_t i = height/2; i < ((height/4)*3); i++) {
        for (size_t j = 0; j < width; j++) {
            uint64_t val = outdata_flat3[((i-(height/2))*width)+j];
    ...
    // Unflatten fourth quarter of output data
    for (size_t i = ((height/4)*3); i < height; i++) {
        for (size_t j = 0; j < width; j++) {
            uint64_t val = outdata_flat4[((i-((height/4)*3))*width)+j];
